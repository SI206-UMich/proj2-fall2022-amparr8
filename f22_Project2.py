...
def check_policy_numbers(data):
    ...
    for item in data:
        ...
        id = item[3]
        #print(id)
        if re.findall('(?:20\d{2}-00\d{4}STR|STR-000\d{4}|(?:P|p)ending|Exempt)', id):
            ignore = re.findall('(?:20\d{2}-00\d{4}STR|STR-000\d{4}|(?:P|p)ending|Exempt)', id)
        else:
            #print(item)
